...
and the particular configuration change the rigidity.
...
becomes a function of only the graph and the embedding dimension.
...
and it is the most widely used library for such purposes.
...
Of these fields, \texttt{source} and \texttt{target} both point to \texttt{Node} objects,
each of which possesses a unique \texttt{id}, its \texttt{x} and \texttt{y} coordinates,
and a map of \texttt{attributes}.
Unique \texttt{id}s are randomly generated,
case-sensitive alphanumeric strings of 5 characters.
\Cref{fig:graphrep} provides a simple example for a line segment.

Note that nodes are passed to \texttt{edges} by reference,
so edges themselves do not have absolute locations.
Furthermore, we use an absolute pixel-based coordinate system to position nodes,
so for convenience,
each node keeps track of its own location on the canvas.
All elements keep track of their own colors,
which represent the number of pebbles in the pebble algorithm
...
We have two \texttt{Node} endpoints and an \texttt{Edge}
connecting them.}
\label{fig:graphrep}
\end{figure}
...
since the performance bottleneck was the GUI rendering code.
...
We attempt to cover the edge with four pebbles,
with one pebble exchange per step.
The number of pebbles on an element
is reflected in our GUI with a spectrum of blue and green,
...
Figure \ref{fig:seg} shows a simplified version of this process for a single segment,
and figure \ref{fig:pebbles} demonstrates multiple phases of the algorithm
for a triangle.
