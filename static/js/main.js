...
(function() {
  var Edge, Graph, Node, PebbleGraph, attachBindings, d3Vars, deleteNodes, disableDraw, drawEdge, drawGraph, drawPebble, getColor, graphVars, hideHelp, initGraph, matrixRank, nodeGenID, onClick, onClickNode, onKeyDown, onKeyUp, onMouseDownNode, onMouseEnterNode, onMouseOutNode, onMouseUp, onMouseUpNode, redraw, resetGraphVars, showAbout, showHelp, toggleNodeSelect, vars,
  ...
  Graph = (function() {
    ...
    Graph.prototype.delNode = function(node) {
      var edge, i, index, k, ref, results;
      index = this.nodes.indexOf(node);
      if (index !== -1) {
        this.nodes.splice(index, 1);
      }
      results = [];
      for (i = k = ref = this.edges.length - 1; ref <= -1 ? k <= -1 : k >= -1; i = ref <= -1 ? ++k : --k) {
        edge = this.edges[i];
        console.log(this.edges);
        if (edge.getTarget() === node || edge.getSource() === node) {
          if (i !== -1) {
            results.push(this.edges.splice(i, 1));
          } else {
            results.push(void 0);
          }
        } else {
