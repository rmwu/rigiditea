...
(function() {
  var Edge, Graph, Node, PebbleGraph, attachBindings, countOccurences, d3Vars, deleteNodes, disableDraw, drawEdge, drawGraph, drawInfinite, drawPebble, getColor, graphVars, hideHelp, initGraph, makePebbleGraph, matrixRank, nodeGenID, onClick, onClickEdge, onClickNode, onKeyDown, onKeyUp, onMouseDownEdge, onMouseDownNode, onMouseEnterNode, onMouseOutEdge, onMouseOutNode, onMouseOverNode, onMouseUp, onMouseUpNode, redraw, resetGraphVars, showAbout, showHelp, toggleEdgeSelect, toggleNodeSelect, vars,
  ...
  attachBindings = function() {
    ...
    $("#ctrlEdge").on("click", function() {
      return drawEdge();
    });
    $("#ctrlDel").on("click", function() {
      return deleteNodes();
    });
    $("#ctrlHelp").on("click", function() {
      return showHelp();
    });
    $(".exit").on("click", function() {
      return hideHelp(this);
    });
  ...
  initGraph = function() {
    ...
    return d3Vars.tooltip = d3.select("#graph").append("div").attr("class", "tooltip").style("opacity", 0);
  };
  ...
  onMouseOverNode = function() {
    var coords, data, pebbles, x, y;
    data = d3.select(this).data()[0];
    coords = d3.mouse(this);
    x = coords[0], y = coords[1];
    if (graphVars.frozen) {
      d3Vars.tooltip.transition().duration(200).style("opacity", 1);
      pebbles = graphVars.graphP.getPebbles(data);
      return d3Vars.tooltip.html(pebbles).style("left", x + "px").style("top", y + "px");
    }
  };
  ...
  drawGraph = function(graph, svg) {
    ...
    svg.selectAll("circle").on("click", onClickNode).on("mousedown", onMouseDownNode).on("mouseup", onMouseUpNode).on("mouseout", onMouseOutNode).on("mouseenter", onMouseEnterNode).on("mouseover", onMouseOverNode);
    return svg.selectAll("line").on("click", onClickEdge).on("mousedown", onMouseDownEdge).on("mouseout", onMouseOutEdge).on("mouseover", onMouseOverNode);
  };
  ...
  drawEdge = function() {
    var source, target;
    if (indexOf.call(graphVars.nodeS, null) < 0 && !graphVars.frozen) {
  ...
  Graph = (function() {
    ...
    Graph.prototype.genericInfDOF = function(dimension) {
      var k, len, node, randomVertexConfig, ref;
      randomVertexConfig = {};
      ref = this.nodes;
      for (k = 0, len = ref.length; k < len; k++) {
        node = ref[k];
  ...
  Edge = (function() {
    function Edge(id1, source1, target1, attr) {
      this.id = id1;
      this.source = source1;
      this.target = target1;
      this.attr = attr;
  ...
  PebbleGraph = (function(superClass) {
    ...
    PebbleGraph.prototype.getPebbles = function(element) {
      var id;
      id = element.id.toString();
      if (id in this.algState.vertexCounts) {
        return this.algState.vertexCounts[id];
      } else {
        return this.algState.edgeCounts[id];
      }
    };
